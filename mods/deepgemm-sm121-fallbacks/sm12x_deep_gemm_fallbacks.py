...
_SM120_MQA_LOGITS_MAX_SCORE_BYTES = 1024 * 1024 * 1024
...
def _fp8_mqa_logits_torch(
    ...
    for head_start in range(0, num_heads, head_chunk_size):
        ...
        q_chunk = q_bf16[:, head_start:head_end, :].transpose(0, 1).contiguous()
        ...
        for k_start in range(0, seq_len_kv, k_chunk_size):
            ...
            scores = torch.matmul(q_chunk, k_t[:, k_start:k_end]).float()
            scores.relu_()
...
def _fp8_mqa_logits_topk_triton(
    ...
    q_values, q_scale = q
    if q_scale is not None:
        raise NotImplementedError("SM120 MQA top-k triton path only supports FP8 Q")

    k_values, k_scales = kv
    k_scales = k_scales.reshape(-1).to(torch.float32)
    seq_len, num_heads, _ = q_values.shape
    ...
    max_chunk_topk = min(topk_tokens, k_chunk_size)
    chunk_values_buf = torch.empty(
        (seq_len, max_chunk_topk), device=q_values.device, dtype=torch.float32
    )
    chunk_indices_buf = torch.empty(
        (seq_len, max_chunk_topk), device=q_values.device, dtype=torch.int64
    )
    chunk_indices_i32 = torch.empty(
        (seq_len, max_chunk_topk), device=q_values.device, dtype=torch.int32
    )
    candidate_values = torch.empty(
        (seq_len, topk_tokens + max_chunk_topk),
        device=q_values.device,
    ...
    next_best_values = torch.empty_like(best_values)
    selected = torch.empty(
        (seq_len, topk_tokens), device=q_values.device, dtype=torch.int64
    )

    ks_allvalid = torch.zeros(seq_len, device=q_values.device, dtype=cu_seqlen_ks.dtype)
    ke_allvalid = torch.empty(seq_len, device=q_values.device, dtype=cu_seqlen_ke.dtype)

    for k_start in range(0, seq_len_kv, k_chunk_size):
        k_end = min(k_start + k_chunk_size, seq_len_kv)
        ke_allvalid.fill_(k_end - k_start)
...
def fp8_fp4_mqa_topk_indices(
    ...
    try:
        ...
        logger.warning(
            "SM120 topk triton path failed (%s); falling back to torch", exc
        )
